...
import { writeChallengePointsForBadge } from "../utils/challengeRankingPoints";
...
export async function evaluateChallenges(scoreData: ScoreData): Promise<void> {
  ...
  try {
    ...
    console.log(`🎯 Evaluating ${activeChallenges.length} challenges for ${userId}`);
    ...
    for (const challengeId of activeChallenges) {
      try {
        ...
        if (earned) badgesEarned.push(challengeId);
      } catch (err) {
...
async function evaluatePar3(
  ...
  for (let i = 0; i < score.holesCount; i++) {
    if (score.holePars[i] === 3) par3Holes.push(score.holeScores[i]);
  }
  if (par3Holes.length === 0) return false;

  const previousHoles = participant.totalPar3Holes || 0;
  const totalHoles = previousHoles + par3Holes.length;
  const totalScore = (participant.totalPar3Score || 0) + par3Holes.reduce((s, h) => s + h, 0);
  ...
  if (totalHoles >= 50 && currentAverage <= participant.targetThreshold) {
    updates.earned = true;
  ...
  await checkProgressMilestones(score.userId, "par3", totalHoles, 50, previousHoles, `${totalHoles}/50 holes`);
  await ref.update(updates);
  return false;
}
...
async function evaluateFIR(
  ...
  const totalHit = (participant.totalFairwaysHit || 0) + score.fairwaysHit;
  const totalPossible = (participant.totalFairwaysPossible || 0) + (score.fairwaysPossible || 0);
  ...
  if (totalRounds >= 10 && currentPct >= participant.targetThreshold) {
    updates.earned = true;
    updates.earnedAt = FieldValue.serverTimestamp();
    await ref.update(updates);
  ...
  await checkProgressMilestones(score.userId, "fir", totalRounds, 10, previousRounds, `${totalRounds}/10 rounds`);
  await ref.update(updates);
  return false;
}
...
async function evaluateGIR(
  ...
  const previousRounds = participant.qualifyingRounds || 0;
  const totalRounds = previousRounds + 1;
  const totalHit = (participant.totalGreensHit || 0) + score.greensHit;
  const totalPossible = (participant.totalGreensPossible || 0) + (score.greensPossible || score.holesCount);
  const currentPct = totalPossible > 0 ? (totalHit / totalPossible) * 100 : 0;
  ...
  await checkProgressMilestones(score.userId, "gir", totalRounds, 10, previousRounds, `${totalRounds}/10 rounds`);
  await ref.update(updates);
  return false;
}
...
async function evaluateBirdieStreak(
  ...
  const newBest = Math.max(participant.bestStreak || 0, longestStreak);
  const updates: Record<string, any> = { bestStreak: newBest };

  if (newBest >= participant.targetThreshold) {
    updates.earned = true;
    updates.earnedAt = FieldValue.serverTimestamp();
    await ref.update(updates);
...
async function evaluateIronPlayer(
  ...
  const newCount = score.grossScore < targetScore ? currentCount + 1 : 0;

  const updates: Record<string, any> = { consecutiveCount: newCount };
  ...
  await checkProgressMilestones(score.userId, "iron_player", newCount, 5, currentCount, `${newCount}/5 consecutive rounds`);
  await ref.update(updates);
  return false;
}
...
async function evaluateDTP(
  ...
  for (const measurement of score.dtpMeasurements) {
    const courseRef = db.collection("challenges").doc("dtp").collection("courses").doc(courseId);
    ...
    if (!courseDoc.exists) {
      ...
      console.log(`📍 DTP: ${userId} set designated hole #${measurement.hole} at ${courseId} — ${measurement.distance}ft`);
    } else {
      ...
      if (measurement.distance < courseData.currentDistance) {
        ...
        if (courseData.currentHolderId !== userId) {
          await notifyPinLost(courseData.currentHolderId, userId, courseData.courseName || "", courseData.designatedHole, measurement.distance, courseData.currentDistance);
        ...
        claimedDistance = measurement.distance;
        console.log(`📍 DTP: ${userId} claimed pin at ${courseId} hole #${measurement.hole} — ${measurement.distance}ft`);
      }
    }
  }
  ...
  if (pinClaimed) {
    ...
    try {
      ...
      await writeDTPClaimedActivity(userId, dtpUserData?.displayName || "Someone", dtpUserData?.avatar || null, claimedCourseName, claimedHole, claimedDistance, dtpUserData?.regionKey || "");
    } catch (feedErr) {
      console.warn("⚠️ Failed to write DTP claimed feed activity:", feedErr);
    }

    return await updateDTPPinCount(userId);
  }
...
async function updateDTPPinCount(userId: string): Promise<boolean> {
  const coursesSnap = await db
    .collection("challenges").doc("dtp").collection("courses")
  ...
  const participantRef = db.collection("challenges").doc("dtp").collection("participants").doc(userId);
  const participantDoc = await participantRef.get();
  ...
  const updates: Record<string, any> = { pinsHeld, coursesWithPins };
  ...
  await db.collection("users").doc(userId).update({ dtpPinsHeld: pinsHeld });
...
async function checkProgressMilestones(
  ...
  try {
    ...
    for (const milestone of [{ threshold: 50, label: "halfway" }, { threshold: 75, label: "75%" }]) {
...
async function awardBadges(userId: string, newBadgeIds: string[]): Promise<void> {
  try {
    ...
    for (const badgeId of newBadgeIds) {
      ...
      // ✅ Wire into ST Power Ranking
      await writeChallengePointsForBadge(userId, badgeId);
    ...
    for (const badge of earnedBadges) {
      ...
      if (badge === "dtp") { if (dtpPinsHeld > 0) activeBadgeCount++; continue; }
      activeBadgeCount++;
    }
    ...
    for (const tier of CUMULATIVE_TIERS) {
      if (activeBadgeCount >= tier.requiredBadges && !alreadyEarnedTiers.includes(tier.id)) {
    ...
    if (newTiers.length > 0) {
      ...
      for (const tierId of newTiers) {
        ...
        // ✅ Wire tier badges into ST Power Ranking
        await writeChallengePointsForBadge(userId, tierId);
...
export async function evaluateAceHunter(userId: string): Promise<void> {
  try {
    ...
    const participantRef = db.collection("challenges").doc("ace").collection("participants").doc(userId);
    const participantDoc = await participantRef.get();
    if (!participantDoc.exists) return;

    if (participantDoc.data()!.earned) return;
